...
Sub Globals
	...
	Private iMeterID As Int
	'Findings
	Private txtPrevRdg As EditText
	Private txtCurrentRdg As EditText
...
Sub Activity_Create(FirstTime As Boolean)
	...
	scvMain.Panel.LoadLayout("MCJOFindings")
	scvMain.Panel.Height = pnlMain.Height
	
	GlobalVar.CSSubTitle.Initialize.Size(12).Append($"Accomplishment Form"$).PopAll
	GlobalVar.CSTitle.Initialize.Size(15).Bold.Append(GlobalVar.SelectedJODesc & $" JO"$).PopAll
...
Private Sub ClearUI
	...
	txtCurrentRdg.Background = cdRemBorder
	txtMeterFindings.Background = cdRemBorder
	txtActionTaken.Background = cdRemBorder
...
Private Sub FillJORecord (iJOID As Int)
	...
	Try
		...
		If RS.RowCount > 0 Then
			...
			iMeterID = RS.GetInt("MeterID")
			lblMeterNo.Text = DBaseFunctions.GetCodeByID("MeterNo", "tblMeters","MeterID", iMeterID)
