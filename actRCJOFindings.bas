...
'	Private lblMeterNo As Label
	...
	'Findings
	Private chkNewMeter As CheckBox
	Private cboMeterBrand As ACSpinner
	Private txtMeterNo As EditText
	Private sMeterNo As String
	Private sMeterBrand As String
	Private dMeterID As Double

	Private lPrevRdg As Long
	Private lFinalRdg As Long
	Private lAddCons As Long
...
Sub Activity_Create(FirstTime As Boolean)
	...
	scvMain.Panel.LoadLayout("RCJOFindings")
	scvMain.Panel.Height = pnlMain.Height
	
	GlobalVar.CSSubTitle.Initialize.Size(12).Append($"Accomplishment Form"$).PopAll
	GlobalVar.CSTitle.Initialize.Size(15).Bold.Append($"Reconnection JO"$).PopAll
...
Private Sub FillJORecord (iJOID As Int)
	...
	Try
		...
		If rsJONCDetails.RowCount > 0 Then
			...
			lPrevRdg = GlobalVar.SF.Val(rsJONCDetails.GetString("PrevRdg"))
			lFinalRdg = GlobalVar.SF.Val(rsJONCDetails.GetString("FinalRdg"))
		Else
			...
			Return
		End If
		sMeterNo = DBaseFunctions.GetCodeByID("MeterNo", "tblMeters","MeterID", dMeterID)
		sMeterBrand = DBaseFunctions.GetCodeByID("BrandName", "tblMeters","MeterID", dMeterID)
		lAddCons = lFinalRdg - lPrevRdg

		LogColor($"Prev Rdg: "$ & lPrevRdg, Colors.Red)
		LogColor($"Final Rdg: "$ & lFinalRdg, Colors.White)
		LogColor($"Add Cons: "$ & lAddCons, Colors.Yellow)
'		lblMeterNo.Text = sMeterNo
'		chkNewMeter.Checked = True
	Catch
		Log(LastException)
	End Try
	rsJONCDetails.Close
	...
	Try
		...
		If rsPipeSize.RowCount > 0 Then
			For i = 0 To rsPipeSize.RowCount - 1
				...
				cboPipeSize.Add(rsPipeSize.GetString("SizeDesc"))
			Next
		End If
	Catch
		ToastMessageShow("Unable to Load Pipe Type due to " & LastException.Message,False)
...
Private Sub ValidateEntries () As Boolean
	Try
		...
		If GlobalVar.SF.Len(txtInitRdg.Text) <= 0 Then
			RequiredMsgBox($"ERROR"$, $"Unable to Accomplshed JO due to No specified Meter's Initial Reading."$)
...
Private Sub UpdateJO (iJOID As Int)
	Dim sMeterBrandName, sMeterNum, sInitRdg, sPrevRdg, sFinalRdg, sRemarks As String
	Dim dOldMeterID, dNewMeterID As Double
	...
	'Findings
	If chkNewMeter.Checked = True Then
		sMeterBrand = cboMeterBrand.SelectedItem
		sMeterNo = txtMeterNo.Text
		sInitRdg = txtInitRdg.Text
		dOldMeterID = dMeterID
		dNewMeterID = DBaseFunctions.GetIDByCode("MeterID","tblMeters", "MeterNo", txtMeterNo.Text)
	Else
		sMeterBrand = sMeterBrand
		sMeterNo = sMeterNo
		sInitRdg = txtInitRdg.Text
		dOldMeterID = dMeterID
		dNewMeterID = dMeterID
	End If
	...
	Try
		...
		Starter.strCriteria = "UPDATE tblJOs " & _
						  "SET JOStatus = ? , DateFinished = ?, AccomplishedBy = ?, SigFileName = ?, PostedAt = ?, PostedOn = ? " & _
						  "WHERE JOID = " & iJOID
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array As String(3, txtDateTimeFinished.Text, sAccomplishedBy, SigFolderName & "/" & SigFilename, sDatePosted, sLocation))
		
		'Insert to SAS Findings Table
		Starter.strCriteria = "INSERT INTO tblJORCFindings VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array(GlobalVar.SelectedJOID, dOldMeterID, sPrevRdg, sFinalRdg, dNewMeterID, sInitRdg, lAddCons, sRemarks))
		
		'Insert to JO Water Loss Table
		Starter.strCriteria = "INSERT INTO JOWaterLoss VALUES (" & Null & ", ?, ?, ?, ?, ?, ?, ?)"
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array(GlobalVar.SelectedJOID, $"RC"$, sPipeType, sPipeSize, iPSI, iMinutes, dWaterLoss))
...
Sub chkNewMeter_CheckedChange(Checked As Boolean)
	If Checked Then
		txtMeterNo.Enabled = False
		txtMeterNo.Text = sMeterNo
		cboMeterBrand.Enabled = False
		cboMeterBrand.SelectedIndex = cboMeterBrand.IndexOf(sMeterBrand)
		txtInitRdg.Text = lFinalRdg
		txtInitRdg.Enabled = False
	Else
		txtMeterNo.Text = ""
		txtMeterNo.Enabled = True
		cboMeterBrand.SelectedIndex = 1
		cboMeterBrand.Enabled = True
		txtInitRdg.Text = "0"
		txtInitRdg.Enabled = True
	End If
End Sub

Sub cboMeterBrand_ItemClick (Position As Int, Value As Object)
