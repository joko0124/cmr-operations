...
Sub Activity_Create(FirstTime As Boolean)
	...
	If GlobalVar.SelectedJOCatCode = "DC-CR" Then
		scvMain.Panel.LoadLayout("DCCRJOFindings")
		GlobalVar.CSTitle.Initialize.Size(15).Bold.Append($"Disconnection-Customer Request JO"$).PopAll
	Else
		scvMain.Panel.LoadLayout("DCDAJOFindings")
...
Sub Activity_PermissionResult (Permission As String, Result As Boolean)
	If Permission = Starter.RTP.PERMISSION_ACCESS_COARSE_LOCATION Then
		Result = True
		LogColor("APP Can use your location.", Colors.Yellow)
	Else
		Result = False
	End If
	Log (Permission)
	...
	If Result Then
		...
		Select SettingsResult.GetLocationSettingsStatus.GetStatusCode
			...
			Case sc.RESOLUTION_REQUIRED
				...
				Dim rs As ResumableSub = CallSub2(MainScreen, "ShowResolutionDialog", SettingsResult.GetLocationSettingsStatus)
	...
	Try
		...
		If rsPipeSize.RowCount > 0 Then
			For i = 0 To rsPipeSize.RowCount - 1
				...
				cboPipeSize.Add(GlobalVar.SF.Proper(rsPipeSize.GetString("SizeDesc")))
			Next
		End If
	Catch
		ToastMessageShow("Unable to Load Pipe Type due to " & LastException.Message,False)
...
Private Sub UpdateJO (iJOID As Int)
	...
	SigFilename = "DC-CR-"& lblJONum.Text & "-" & lblAppNum.Text & ".jpg"
...
Private Sub DispInfoMsg(sTitle As String, sMsg As String)
	MatDialogBuilder.Initialize("DispInormationMsg")
...
Private Sub DispInormationMsg_ButtonPressed(mDialog As MaterialDialog, sAction As String)
	Select Case sAction
...
Private Sub flp_LocationChanged (Location1 As Location)
	...
	GlobalVar.Lon = $"$1.3{Location1.Longitude}"$
End Sub
