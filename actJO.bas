...
'	Private lblJOType As Label
	...
	Type JORecords (JOID As Int, JONum As String, JOCatCode As String, JOStatus As Int, RefID As Int, RefNo As String, _
...
Sub FilterBy_ItemClicked (Item As ACMenuItem)
	...
	FilterJoList(GlobalVar.SelectedJOCatCode, Item.Id)
End Sub
...
Private Sub FilterJoList (sSelectedJOCat As String, iStatus As Int)
	Dim SenderFilter As Object
	...
	Select JORec.JOStatus
		...
		Case 2	'STARTED
			Select JORec.JOCatCode
				Case "SAS"
					StartActivity(actSASJOFindings)
				Case "NC"
	...
	For i = Max(0, FirstIndex - ExtraSize) To Min(LastIndex + ExtraSize, clvJOs.Size - 1)
		...
		If i > FirstIndex - ExtraSize And i < LastIndex + ExtraSize Then
			If Pnl.NumberOfViews = 0 Then 'Add each item/layout to the list/main layout
				Dim JORec As JORecords = clvJOs.GetValue(i)
				Pnl.LoadLayout("JOInfo")
				If GlobalVar.SelectedJOCatCode = "SAS" Or GlobalVar.SelectedJOCatCode = "NC"  Then
					lblRefTitle.Text = $"Application No.: "$
				Else
