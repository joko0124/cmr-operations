...
Sub Globals
	...
	Type JOSummary (JOCatCode As String, JODesc As String, TotalPending As Int, TotalOnGoing As Int, TotalAccomplished As Int, TotalCancelled As Int, JOTotals As Int)
...
Sub Activity_Create(FirstTime As Boolean)
	...
	If FirstTime Then
		...
		FillJOList(GlobalVar.UserID, GlobalVar.BranchID, GlobalVar.SelectedJOCatCode)
	End If
...
#Region JO Lists
Private Sub FillJOList (iUserID As Int, iBranchID As Int, sJOCatCode As String)
	...
	Try
		Starter.strCriteria = "SELECT sum(CASE WHEN JOs.JOStatus = 1 THEN 1 ELSE 0 END) AS PendingJOs, " & _
						  ...
						  "count(JOs.JOID) AS TotalJOs, JOs.JoDesc " & _
						  ...
						  "AND JOs.JOCatCode = '" & sJOCatCode & "' " & _
						  "GROUP BY JOs.JoDesc ORDER BY JOCat.id ASC"
...
'				JOCount.JOCatCode = GlobalVar.SF.Upper(RS.GetString("JOCatCode"))
				JOCount.JODesc = GlobalVar.SF.Proper(RS.GetString("JoDesc"))
			...
			If JOCount.JOTotals > 1 Then
				lblCount.Text = RS.RowCount & " " & GlobalVar.SelectedJODesc & $" Job Orders Found"$
			Else
				lblCount.Text = RS.RowCount & " " & GlobalVar.SelectedJODesc & $" Job Order Found"$
			End If
		Else
			Log(LastException)
		End If
...
Sub clvJOs_ItemClick (Index As Int, Value As Object)
	Dim JOCount As JOSummary = Value
...
'	GlobalVar.SelectedJOCatCode = JOCount.JOCatCode
	GlobalVar.SelectedJODesc = JOCount.JODesc
	StartActivity(actJOReasons)
End Sub
	...
	For i = Max(0, FirstIndex - ExtraSize) To Min(LastIndex + ExtraSize, clvJOs.Size - 1)
		...
		If i > FirstIndex - ExtraSize And i < LastIndex + ExtraSize Then
			If Pnl.NumberOfViews = 0 Then 'Add each item/layout to the list/main layout
				Dim JORec As JOSummary = clvJOs.GetValue(i)
...
Sub txtSearch_TextChanged (Old As String, New As String)
	...
	If New.Length = 0  Then
		...
		Starter.strCriteria = "SELECT sum(CASE WHEN JOs.JOStatus = 1 THEN 1 ELSE 0 END) AS PendingJOs, " & _
						  ...
						  "AND JOs.JOCatCode = '" & GlobalVar.SelectedJOCatCode & "' " & _
						  "GROUP BY JOs.JoDesc ORDER BY JOCat.id ASC LIMIT 100"
	Else
		Starter.strCriteria = "SELECT sum(CASE WHEN JOs.JOStatus = 1 THEN 1 ELSE 0 END) AS PendingJOs, " & _
						  "sum(CASE WHEN JOs.JOStatus = 2 THEN 1 ELSE 0 END) AS OnGoingJOs, " & _
						  "sum(CASE WHEN JOs.JOStatus = 3 THEN 1 ELSE 0 END) AS AccomplishedJOs, " & _
