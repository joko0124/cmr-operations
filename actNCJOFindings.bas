...
Sub Globals
	...
	'Findings
	Private cboBookNo As ACSpinner
	Private txtSeqNo As EditText
	Private cboMeterBrand As ACSpinner
	Private txtMeterNo As EditText
...
Sub Activity_Create(FirstTime As Boolean)
	...
	scvMain.Panel.LoadLayout("NCJOFindings")
	scvMain.Panel.Height = pnlMain.Height
	
	GlobalVar.CSTitle.Initialize.Size(15).Bold.Append($"New Connection JO"$).PopAll
...
Private Sub ClearUI
	...
	cboBookNo.Background = cdRemBorder
	txtSeqNo.Background = cdRemBorder
	cboMeterBrand.Background = cdRemBorder
	txtMeterNo.Background = cdRemBorder
...
Private Sub FillBooks
	...
	cboBookNo.Clear
	Try
		Starter.strCriteria = "SELECT * FROM tblBooks"
		rsBooks = Starter.DBCon.ExecQuery(Starter.strCriteria)
		If rsBooks.RowCount > 0 Then
			For i = 0 To rsBooks.RowCount - 1
				rsBooks.Position = i
				cboBookNo.Add(GlobalVar.SF.Proper(rsBooks.GetString("BookCode")) & $" - "$ & GlobalVar.SF.Proper(rsBooks.GetString("BookDesc")))
			Next
		End If
	Catch
		ToastMessageShow("Unable to Books Type due to " & LastException.Message,False)
		Log(LastException)
	End Try
	rsBooks.Close
End Sub

Private Sub FillMeterBrands
	Dim rsMeterBrands As Cursor
	cboMeterBrand.Clear
	Try
		Starter.strCriteria = "SELECT * FROM cons_meter_brand"
		rsMeterBrands = Starter.DBCon.ExecQuery(Starter.strCriteria)
		If rsMeterBrands.RowCount > 0 Then
			For i = 0 To rsMeterBrands.RowCount - 1
				rsMeterBrands.Position = i
				cboMeterBrand.Add(GlobalVar.SF.Proper(rsMeterBrands.GetString("BrandName")))
			Next
		End If
	Catch
		ToastMessageShow("Unable to Books Type due to " & LastException.Message,False)
...
Private Sub ValidateEntries () As Boolean
	...
	Try
		If GlobalVar.SF.Len(cboBookNo.SelectedItem) <= 0 Then
			RequiredMsgBox($"ERROR"$, $"Unable to Accomplshed JO due to No selected Book."$)
			cboBookNo.RequestFocus
			Return False
		End If
	
		If GlobalVar.SF.Len(txtSeqNo.Text) <= 0 Then
			RequiredMsgBox($"ERROR"$, $"Unable to Accomplshed JO due to No specified Sequence Number."$)
			txtSeqNo.RequestFocus
			Return False
		End If

		If GlobalVar.SF.Len(cboMeterBrand.SelectedItem) <= 0 Then
			RequiredMsgBox($"ERROR"$, $"Unable to Accomplshed JO due to No selected Meter Brand."$)
			cboMeterBrand.RequestFocus
			Return False
		End If
	
		If GlobalVar.SF.Len(txtMeterNo.Text) <= 0 Then
			RequiredMsgBox($"ERROR"$, $"Unable to Accomplshed JO due to No specified Meter Number."$)
			txtMeterNo.RequestFocus
			Return False
		End If
...
Sub cboBookNo_ItemClick (Position As Int, Value As Object)
	txtSeqNo.RequestFocus
End Sub
...
Sub txtSeqNo_FocusChanged (HasFocus As Boolean)
	txtSeqNo.SelectAll
End Sub

Sub txtSeqNo_EnterPressed
	txtMeterNo.RequestFocus
End Sub

Sub txtMeterNo_FocusChanged (HasFocus As Boolean)
	txtMeterNo.SelectAll
End Sub

Sub txtMeterNo_EnterPressed
	txtInitRdg.RequestFocus
End Sub

Sub txtInitRdg_FocusChanged (HasFocus As Boolean)
	txtInitRdg.SelectAll
End Sub

Sub txtInitRdg_EnterPressed
	txtPSI.RequestFocus
End Sub
...
Private Sub UpdateJO (iJOID As Int)
	Dim sBookNo, sSeqNo, sMeterBrand, sMeterNo, sRemarks As String
	Dim iInitRdg As Int
	Dim iBookID As Int
	...
	'Findings
	sBookNo = cboBookNo.SelectedItem
	sSeqNo = txtSeqNo.Text
	sMeterBrand = cboMeterBrand.SelectedItem
	sMeterNo = txtMeterNo.Text
	iInitRdg = txtInitRdg.Text
	
	iBookID = DBaseFunctions.GetIDByCode("BookId", "tblBooks", "BookCode", GlobalVar.SF.Left(sBookNo,3))
	dMeterID = DBaseFunctions.GetIDByCode("MeterId", "tblMeters", "MeterNo", txtMeterNo.Text)
	...
	SigFilename = "NC -"& lblJONum.Text & "-" & lblAppNum.Text & ".jpg"
	...
	Try
		...
		Starter.strCriteria = "UPDATE tblJOs " & _
						  "SET BookID = ?, MeterID = ?, PrevRdg = ?, JOStatus = ? , DateFinished = ?, AccomplishedBy = ?, SigFileName = ?, PostedAt = ?, PostedOn = ? " & _
						  "WHERE JOID = " & iJOID
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array As String(iBookID, dMeterID, iInitRdg, 3, txtDateTimeFinished.Text, sAccomplishedBy, SigFolderName & "/" & SigFilename, sDatePosted, sLocation))
		
		'Insert to SAS Findings Table
		Starter.strCriteria = "INSERT INTO tblJONCFindings VALUES (?, ?, ?, ?, ?, ?)"
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array(GlobalVar.SelectedJOID, dMeterID, iInitRdg, iBookID, sSeqNo, txtRemarks.Text))
		
		'Insert to JO Water Loss Table
		Starter.strCriteria = "INSERT INTO JOWaterLoss VALUES (" & Null & ", ?, ?, ?, ?, ?, ?, ?)"
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array(GlobalVar.SelectedJOID, $"NC"$, sPipeType, sPipeSize, iPSI, iMinutes, dWaterLoss))
