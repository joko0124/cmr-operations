...
Sub Globals
	...
	Private chkDefaultTimeOn As CheckBox
	...
	Private TotDrainHrs, TotDrainCum, totPSI As Double
	Private PowerSourceID As Int
	Private TotOpHrs As Float
	
	Private PumpPowerStatus As Int
	
	Private Spinner1 As Spinner
	Private B4ADropDownView1 As B4ADropDownView
...
Sub Activity_Create(FirstTime As Boolean)
	MyScale.SetRate(0.5)
	Activity.LoadLayout("NewPumpTimeRecords2")
...
Sub btnSave_Click
	...
	If chkDrain.Checked = True Then
		TotDrainHrs = txtDuration.Text
		TotDrainCum = txtDrainCum.Text
	Else
		TotDrainHrs = 0
		TotDrainCum = 0
	End If
	...
	If GlobalVar.blnNewTime = True Then
		...
		If DBaseFunctions.IsFuturisticTime(GlobalVar.TranDate,mskTimeOn.Text) = True Then
			RequiredMsgBox($"E R R O R"$, $"Unable to Add New Pump Time record due to specified time is too soon."$)
			mskTimeOn.RequestFocus
			Return
		End If

		If DBaseFunctions.IsTimeOnOverlapping(sTimeOn, GlobalVar.TranHeaderID) = True Then
		...
		If Success Then
			...
			If Matcher1.Find Then
				...
				If AmPm = "AM" Then
					If iHrs = 12 Then
						iHrOn = 0
						mskTimeOn.Text = $"0"$ & iHrOn & $":"$ & sMin
					Else
						iHrOn = iHrs
						If GlobalVar.SF.Len(GlobalVar.SF.Trim(iHrOn)) = 1 Then
							mskTimeOn.Text = $"0"$ & iHrOn & $":"$ & sMin
						Else
							mskTimeOn.Text = iHrOn & $":"$ & sMin
						End If
					End If
				Else
					If iHrs < 12 Then
			...
			If sTimeOff = "" Or GlobalVar.SF.Len(sTimeOff) <= 0 Then
				
			Else
				DateTime.TimeFormat = "HH:mm"
				Matcher1 = Regex.Matcher("(\d\d):(\d\d) (\S\S)", sTimeOff)
				If Matcher1.Find Then
					Dim iHrs, iMins As Int
					Dim AmPm As String
					Dim sMin1 As String
			...
			LogColor(sTimeOn,Colors.Yellow)
			LogColor(sTimeOff,Colors.Yellow)
...
Private Sub IsValidEntries() As Boolean
	...
	Try
		If GlobalVar.SF.Len(GlobalVar.SF.Trim(mskTimeOn.Text)) <= 0 Or mskTimeOn.Text = "__:__" Then
			RequiredMsgBox($"ERROR"$, $"Pump Time cannot be blank!"$)
			mskTimeOn.RequestFocus
			Return False
...
Private Sub SaveTransHeader() As Boolean
	...
	Try
		...
		Starter.DBCon.ExecNonQuery2("INSERT INTO TranHeader VALUES (" & Null & ", ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", _
							   Array As Object(GlobalVar.BranchID, GlobalVar.PumpHouseID, GlobalVar.TranDate, $"0"$, $"0"$, TotDrainCum, TotDrainHrs, $"0"$, $"0"$, $"0"$, $"0"$, $"0"$, $"0"$, GlobalVar.UserID, sAddedAt, Null, Null, $"0"$, $""$, $""$))
...
Private Sub InsertNewPumpTime() As Boolean
	...
	Try
		Starter.DBCon.ExecNonQuery2("INSERT INTO OnOffDetails VALUES (" & Null & ", ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", _
													  	Array As Object(GlobalVar.TranHeaderID, sTimeOn, $""$, $"0"$, PowerSourceID, txtPSI.Text, TotDrainHrs, TotDrainCum, sRemarks, $""$, $"0"$, GlobalVar.UserID, sDateTime, sLocation, Null, Null, $""$, $"0"$, $""$))
		'HeaderID, PumpOnTime, PumpOffTime, TotOpHrs, PowerSourceID, DrainPSI, DrainTime, DrainCum, TimeOnRemarks, TimeOffRemarks, WasUploaded, AddedBy, AddedAt, AddedOn, ModifiedBy, ModifiedAt, ModifiedOn, UploadedBy, UploadedAt
		Starter.DBCon.TransactionSuccessful
		bRetVal = True
	Catch
		Log(LastException)
...
Private Sub UpdatePumpTime(iTranHeaderID As Int, iDetailID As Int) As Boolean
	...
	Try
		...
		Starter.DBCon.ExecNonQuery2(Starter.strCriteria, Array As String(sTimeOn, TotOpHrs, PowerSourceID, TotDrainHrs, TotDrainCum, txtOnRemarks.Text, GlobalVar.UserID, sDateTime))
		Starter.DBCon.TransactionSuccessful
		bRetVal = True
	Catch
		Log(LastException)
...
'	ToastMessageShow("Positive Button Clicked!",False)
	...
	Alert.Initialize.Dismiss2
	Select GlobalVar.blnNewTime 'New or Edit
		Case True
			GlobalVar.TranHeaderID = DBaseFunctions.GetHeaderID(GlobalVar.PumpHouseID, GlobalVar.TranDate)
			LogColor($"Header ID: "$ & GlobalVar.TranHeaderID, Colors.Yellow)
			...
			If GlobalVar.TranHeaderID = 0 Or Not(DBaseFunctions.IsTransactionHeaderExist(GlobalVar.PumpHouseID, GlobalVar.TranDate)) Then
				...
				If Not(InsertNewPumpTime) Then Return
				DBaseFunctions.UpdatePumpPowerStatus (1, GlobalVar.PumpHouseID)
				ShowSaveSuccess
			Else
'				If DBaseFunctions.IsTimeOnOverlapping(sTimeOn, GlobalVar.TranHeaderID) = True Then
'					RequiredMsgBox($"E R R O R"$,$"Unable to Add New Pump Time record due to it will Overlap existing records."$)
